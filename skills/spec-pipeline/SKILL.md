---
name: spec-pipeline
description: Graduate a plan into a numbered spec pipeline in docs/research/{topic}/ — an umbrella plus one self-contained spec per planned seat. Use when a research idea is "really getting into it" and a plan has come through (e.g. plan mode) and you want to commit it as specs instead of executing it directly, or run spec-pipeline.
---
...
> ⚙ **v1 PLANE (still running) — chain mode.** This skill was `spec-chain` until the cutover, and the v1
> plane still calls a pipeline a *chain*, a seat a *child*, and the effort's id a *coordinator id*. The
> artifacts are identical; only the words moved. If you land here from an older doc or an older habit,
> `/spec-chain` is a stub that points at this file.
...
| 2 — graduate to specs | research exists, you're getting into it (plan mode) | **`spec-pipeline`** | numbered specs in `research/{topic}/`; **the plan is now ephemeral** |
| 3 — build | per spec | implement → `docs-write` | working doc that `Source:`s its spec |
...
1. **Gather the inputs.** You need the *plan* (e.g. from plan mode) and the *research doc(s)* behind it.
   Pick the `{topic}` folder name (kebab-case; reuse an existing `research/{topic}/` if extending one).

2. **Decide the slices.** Each numbered spec is **one self-contained build seat**. Cut the plan along
   session boundaries — a slice a single session can build *and* verify. Put them in build order.
...
2b. **Decide the pipeline's SHAPE — ask the human both questions, with your recommendation.** The bookends
   ...
   **"Do you want DOCS-PROCESS as a row?"** In-pipeline gives you a zero-touch close: living docs patched
   ...
   is only **about to be** merged, and the `research/{topic}` → `archive/{topic}` rename rides the same
...
3. **Write the specs**, using the spec-doc template in `docs/_meta/docs-workflow.md`. Each per-seat
   ...
   **Six things every slice carries**, each earned by a session that did not have it:
   ...
   - **The population, not an instance — and DERIVE it wherever you can.** *"Rename the verb at its 9 call
     ...
     **A written count decays silently; a derived one cannot.** Prefer a spec that says *"derive the set
     with this command and report what you got"* over one that hands over a number. Three receipts from a
     single effort: a session's own derived entry-point dump contradicted a doc calling something *"the only
     emitter"* **without anyone going looking**; a fixture's true size was 2,232 against a documented
     "1,100+" **and against a 4,130 correction that had inherited the defect it was correcting**; and a
     census divergence carried through three artifacts turned out never to have existed.
   ...
   **Cite symbols, not line numbers.** A spec outlives many commits, so name the symbol and use the line
   as a hint. A line number is a comment, not an address: one anchor drifted four times inside a single
   effort, invalidated each time by a seat documenting why the line mattered, and the only citations that
   survived were symbol-named.
...
That includes the **kick text**: write the parent's task string here, ready to paste, so igniting requires
no authoring at all.

```markdown
# {Topic} — Ignition Brief (the parent's standing orders)
...
## Birth
Topic · spec path · receipt prefix · attended/unattended · branch `nas/chain-{coordinatorId}`.
...
the parent needs to know whether a review precedes its retro. If there is a review row, add:
**its seed carries no parent beliefs, no provenance, no already-settled framing** · a FIXIT row may
follow it (one maximum, parent-ruled from the review's named candidates) · **the retro comes after
the review and after any fixit**, never before. If docs-process is a row, note that its living-doc
patches are contingent on the merge.

## The parent's beliefs, with provenance
...
A brief that calls a fixed defect broken teaches the parent to work around nothing.

If this pipeline CHANGES the machinery it runs on — the ledger, the wake digest, the audit lane, the
sentinel, the gate — say so here, and tell the parent to expect to meet that defect while fixing it.
...
voices, the seat's tldr and the parent's tracker, and the retro's ceremony audit is built on
cross-reading them; a human row recorded only by the coordinator has ONE. **Receipt:** one pipeline's human
row produced the corpus five later rows were specced against AND failed the check that triggered a
fixit — and its entire record was a tracker cell the coordinator wrote, in a run whose own retro logs
sixteen parent errors, twelve caught by someone else.

## The kick — paste this verbatim as the parent's task
...
rides the un-skippable policy suffix the platform supplies; a brief that re-teaches them accretes into
prompt-law that breaks the coordinator it was meant to steer. The brief carries THIS RUN's choreography and
nothing else.

**The brief must be committed to main before the run is kicked.** The coordinator reads it from its
worktree checkout, and the kick text points at it — so an uncommitted brief leaves it with almost nothing.
That makes it ignition's first gate, and it is the reason this file is worth authoring carefully.

## The `project-status.md` template

Not a spec — the **living tracker**, the coordinator's live journal. Ship it with its section
notes intact: they are where the tracker's own rules live, and they render invisibly in the workbench
because they are instructions to the parent, not content for the human reading it.

```markdown
# {Topic} — Project Status

> The parent's ledger. Advanced at every session boundary, never reconstructed at close.
...
<!-- The journal, one prose section per session. The ruling AND its reasoning, recon corrections,
     what the parent verified itself, mid-build directives, anything worth not forgetting.
...
<!-- 🔴 THIS HEADING IS VERBATIM AND MUST NOT BE RENAMED. It is the destination every seat, review
     ...
     unroutable to anyone who does not already know it. RECEIPT: one pipeline renamed this to
     "## Findings", and its review — a seat whose entire value is having none of the run's
     context — filed 15 findings to a name it could not resolve to a place. Fourteen never landed.
     ...
     🔴 A SEAT OR REVIEW NEVER MINTS ITS OWN ID NAMESPACE — a second sequence beside the parent's
...
<!-- The moment behaviour is wrong — including the parent's own slips. -->

## Session-ID ledger
<!-- Seat → CLAUDE_CODE_SESSION_ID → window name. Written as each seat is armed. -->
...
<!-- Row P is handed a LIST, never a directory: docs/working/ is a shared queue, and a seat that infers
     ...
     conditional fixit row adds one. One pipeline budgeted four and closed at five.
     The parent writes each name at the reap that creates it, in the same pass as the SHIPPED row.
...
*is* the decision record; a parallel section splits one narrative across two places. Both were invented
once and never used.
...
`⬜ owed` → `✅ passed` · `❌ failed` (a failure is a finding, not a retry) · `⚪ not exercised` (with the
reason — an honest and useful result) · **`⚠ partly` (with which half held and which did not)**.

**And a row whose subject decomposes into halves owned by different actors is authored as TWO rows.** One
effort asked *"the review row spawns with a seed carrying no coordinator beliefs"* as a single ✅/❌ — and
the truth was that the seed's content was clean (the coordinator's half) while the platform's preset
defeated it (the machine's half). A status had to be invented to record that honestly. **If you can name
two owners while writing the row, write two rows.**
...
- **Self-contained, like `docs-backlog`.** Assume the plan file (`~/.claude/plans/*`), the chat transcript,
  and any subagent reports vanish. Inline their load-bearing content; never point at ephemeral state as the
  source of truth. Cross-links to *other durable docs* are fine.
...
- **Author against the HEAD the run will start from — and know what that does not buy you.** Authoring close
  to HEAD reduces drift; it does not substitute for a seat reading the code. One effort authored all five
  specs at ignition against the exact HEAD they ran on, and recon overturned a load-bearing claim in **all
  five anyway**. So write specs as HYPOTHESES: state assumptions explicitly, because they are the first
  things recon tests, and make them easy to correct.
...
- **Leave one genuine fork OPEN per spec, with an embedded recommendation** (options a/b + "my
  ...
  beaten both the spec's and the parent's designs.
