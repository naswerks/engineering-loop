...
2. **Write/finish the working doc.** If a doc for this effort already exists in `docs/working/`, finish it; otherwise create `docs/working/{topic}-{description}.md`. Use the session-doc template in `docs/_meta/docs-workflow.md`. Required sections:
   ...
   - **Header** — the machine header `docs/_meta/docs-workflow.md` § Doc metadata (repository-owned) prescribes, in that section's own words (`rg -n 'repository-owned' docs/_meta/docs-workflow.md` finds it): the doc's type is working, its status the section's word for in-progress or shipped, its verified date today. `lineage` = the count of provenance links the doc carries (`Source:` + `Builds on` + any `## Lineage` rows).
...
3. **Record the archive target — don't move the file.** Add an `## Archive` line to the doc naming the folder `docs/_meta/docs-workflow.md` § The archive convention (repository-owned) files this effort into (the loop's own is a **topic folder** under `docs/archive/{topic}/`: reuse an existing one; descriptive name, **no numeric prefixes** — the `Builds on` link is the ordering). When the effort has a spec pipeline whose umbrella carries an `Archive:` line, that path is the target. `docs-process` files it there later. The doc itself **stays in `docs/working/`**.
